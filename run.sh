...
exec nice -n-10 ionice -c 1 -n 1 qemu-system-x86_64 -nographic -serial mon:stdio \
    -netdev tap,id=qemu0,vhost=on,queues=6,script=$QEMU_IFUP,downscript=$QEMU_IFDOWN,packed=off -device virtio-net-pci,netdev=qemu0,mq=on,vectors=9 \
    -netdev tap,id=qemu1,vhost=on,queues=6,script=$QEMU_IFUP,downscript=$QEMU_IFDOWN,packed=off -device virtio-net-pci,netdev=qemu1,mq=on,vectors=9 \
    -netdev tap,id=qemu2,vhost=on,queues=6,script=$QEMU_IFUP,downscript=$QEMU_IFDOWN,packed=off -device virtio-net-pci,netdev=qemu2,mq=on,vectors=9 \
    "$@" \
